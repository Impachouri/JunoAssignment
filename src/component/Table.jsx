...
const Table = ({ headings, data, searchQuery, triggerReasonFilter, riskLevelFilter }) => {
    ...
    const filteredData = data.filter((row) =>
        headings.some((heading) => {
            ...
            return valuesToCompare.some((value) => value.includes(normalizedSearchQuery));
        })
    );
                ...
                {filteredData.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                        {headings.map((heading, colIndex) => (
                            <td key={colIndex}>{renderTableCell(heading, row, searchQuery)}</td>
                        ))}
                    </tr>
                ))}
            </tbody>
